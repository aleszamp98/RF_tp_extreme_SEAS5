...
KEY_CDS = "f6df6ab4-1533-4366-b110-042b9c50aa5c"
# Anni da scaricare e cartella di destinazione
ANNI = range(1992, 2023)
CARTELLA_DESTINAZIONE = "SST"
MAX_TENTATIVI = 3
...
# Dataset e template di richiesta (senza la chiave 'year', che aggiungeremo dinamicamente)
...
request_template = {
    ...
    "day": ["01"],
    # Genera automaticamente la stringa da "24" a "5160" con step di 24
    "leadtime_hour": [str(h) for h in range(24, 5160, 24)],
    "data_format": "netcdf",
    "area": [30, -30, -10, 20]
}
...
def scarica_anno(anno):
    ...
    """
    # Copia il template per evitare sovrascritture tra thread
    request = request_template.copy()
    request["year"] = [str(anno)]
    
    file_path = os.path.join(CARTELLA_DESTINAZIONE, f"SEAS5_SST_{anno}.nc")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # Sottomette tutti i task al pool
        futures = {executor.submit(scarica_anno, anno): anno for anno in ANNI}
        
        # Man mano che finiscono, stampa lo stato
        for future in as_completed(futures):
            risultato = future.result()
            # Il log specifico (successo o fallimento) è già gestito nei print della funzione scarica_anno
