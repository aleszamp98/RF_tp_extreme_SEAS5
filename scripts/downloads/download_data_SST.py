...
MAX_RETRIES = 3
MAX_WORKERS = 3  # Simultaneous requests (Note: CDS queues might throttle multiple workers)
...
def download_year(year):
    """Worker function to download SST data for a single year with retry logic."""
    # Copy template to avoid race conditions between threads
    request = REQUEST_TEMPLATE.copy()
    request["year"] = [str(year)]
    
    filepath = OUTPUT_DIR / f"SEAS5_SST_{year}.nc"
    
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            logging.info(f"Starting SST download: year {year} (Attempt {attempt}/{MAX_RETRIES}) -> {filepath}")
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            # Internal logging inside download_year already handles success/error messages
            _ = future.result()
